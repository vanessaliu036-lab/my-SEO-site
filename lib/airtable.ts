...
// OCC_Blog_Posts is the sole runtime source of truth after corpus consolidation.
// The legacy Articles table and its 387-record manifest remain recovery evidence,
// but are no longer queried by the public website.
const PUBLIC_BLOG_TABLES = ['OCC_Blog_Posts'] as const
...
// These are the migrated members of the frozen historical public corpus.
// They remain indexable even if a later editorial SEO gate says rewrite or deduplicate.
...
function recordToPublishedItem(record: AirtableRecord): BlogPost | null {
  ...
  const title = pickField(record.fields, K.title) || 'Untitled'
...
// List, pagination and sitemap generation need metadata only. Article bodies are
// fetched on detail requests, preventing oversized Airtable responses and cache entries.
...
export async function getAllPosts(): Promise<BlogPost[]> {
  ...
  try {
    const records = await fetchTableRecords(PUBLIC_BLOG_TABLES[0])
...
async function fetchRecordById(recordId: string): Promise<AirtableRecord | null> {
  ...
  const tableName = PUBLIC_BLOG_TABLES[0]
  try {
    const res = await fetch(
...
export async function getPostBySlug(urlSlug: string): Promise<BlogPostDetail | null> {
  ...
  const keyLast = canonicalSlugForUrl(normalizeSlugParam(urlSlug))
  ...
  try {
    ...
    let rows: AirtableRecord[] = Array.isArray(data?.records)
    ...
    if (!rows.length) {
      ...
      rows = Array.isArray(data?.records)
        ? (data.records as Array<{ id: string; fields: Record<string, unknown> }>).map((record) => ({
            ...record,
            tableName,
          }))
