...
export async function sendContactNotification(data, { env = process.env, fetchImpl = fetch } = {}) {
  ...
  const jobTitle = cleanSubject(data.jobTitle) || 'Not provided'
  ...
  try {
    const response = await fetchImpl(RESEND_ENDPOINT, {
      ...
      body: JSON.stringify({
        ...
        html:
          `<h1>New OCC website enquiry</h1><p><strong>Name:</strong> ${escapeHtml(name)}</p><p><strong>Job Title / Position:</strong> ${escapeHtml(jobTitle)}</p><p><strong>Company:</strong> ${escapeHtml(company)}</p><p><strong>Phone:</strong> ${escapeHtml(phone)}</p><p><strong>Work Email:</strong> ${escapeHtml(replyTo)}</p><p><strong>Country / Market:</strong> ${escapeHtml(country)}</p><p><strong>Intent:</strong> ${escapeHtml(service)}</p><p><strong>Project / Requirement:</strong></p><p>${escapeHtml(message).replaceAll('\n', '<br>')}</p>`,
      }),
      cache: 'no-store',
