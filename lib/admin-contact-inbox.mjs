...
export async function convertContactLead(recordId, target, { env = process.env, fetchImpl = fetch, now = () => new Date() } = {}) {
  ...
  const source = [
    '/contact',
    lead.sourceMedium || '(unknown)',
    lead.landingPage || '/contact',
    'lead ' + recordId,
  ].join(' · ')
  let createdId
  let linkField
  ...
  if (target === 'account') {
    createdId = await createDestination(ACCOUNT_TABLE_ID, {
      ...
      [ACCOUNT_FIELDS.notes]: [
        lead.interest,
        lead.stage,
        lead.message,
        'Acquisition: ' + (lead.sourceMedium || '(unknown)'),
        'Landing: ' + (lead.landingPage || '/contact'),
        lead.lastTouchPage ? 'Last touch: ' + lead.lastTouchPage : '',
      ].filter(Boolean).join('\n'),
    }, { env, fetchImpl })
