...
test("OCC keeps grouped public routes and adds the commercial Original gateway without retiring SEO depth", () => {
  for (const required of [
    "app/(site)/layout.tsx",
    "app/(site)/page.tsx",
    "app/(site)/about/page.tsx",
    "app/(site)/solutions/page.tsx",
    "app/(site)/original/page.tsx",
    "app/(site)/origins/page.tsx",
...
test("retired PREK origin URL routes to the current Cambodia origin owner", () => {
  ...
  assert.match(nextConfig, /source:\s*['"]\/origins\/prek['"][\s\S]{0,160}?destination:\s*['"]\/origins['"][\s\S]{0,80}?permanent:\s*true/)
...
test("public navigation follows About to Solutions to Original to Contact and hides legacy service entry points", () => {
  ...
  for (const child of [
    "Mission", "Founder", "Manifesto", "Sustainability",
    "Wholesale & Sourcing", "Roasted Coffee Supply", "Roasting Program", "Distribution Partnership",
    "Origin Evidence", "Fine Robusta Cambodia", "Farm & Terroir",
  ]) {
    assert.match(nav, new RegExp(child.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")))
  }
